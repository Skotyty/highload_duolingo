...
| Логи входа             | 34 100 000 в день                   | 0,5 КБ                  | ~17 ГБ (в день)        |
...
4. **cdn.duolingo.com**  
   Сеть доставки контента для статических ресурсов: изображения, аудио, видео и другие файлы.

5. **events.duolingo.com**  
   Сбор и обработка событий для аналитики и логирования пользовательской активности.

6. **notifications.duolingo.com**  
...
Для эффективной доставки трафика используем **GeoDNS**.

- **GeoDNS** определяет географическое положение пользователя и возвращает IP-адрес ближайшего ДЦ.
- Позволяет снизить задержки и улучшить пользовательский опыт.

### Схема Anycast-балансировки

Для сервисов, требующих высокой скорости и устойчивости (например, CDN), используем **Anycast**.

- **Anycast** позволяет нескольким ДЦ объявлять один и тот же IP-адрес.
- Запрос пользователя направляется в ближайший по сети ДЦ.

## 4. Локальная балансировка нагрузки

### L3 и L7 Балансировка

Для локальной балансировки запросов используется комбинация L3 и L7 балансировки для эффективного управления входящим трафиком.

#### L3 Балансировка:
- На уровне сети (L3) применяется балансировка с использованием **IP Hashing**.
  Этот метод распределяет трафик на основе IP-адреса клиента, что позволяет отправлять повторные запросы одного и того же клиента на один и тот же сервер, что помогает сохранить сессии и уменьшить нагрузку на другие серверы.
  
- Для мониторинга доступности нод используется **Keepalived**. Он следит за состоянием серверов и при отказе одного из них перенаправляет трафик на здоровые ноды, обеспечивая отказоустойчивость.

#### L7 Балансировка:
- На уровне приложений (L7) используется **Nginx** как прокси-сервер. Он распределяет HTTP-запросы между серверами-бэкендами на основе маршрутов URL и заголовков запросов.
 Nginx также может кэшировать часто запрашиваемый контент, например уроки, что снижает нагрузку на бекенд.

- Важно также реализовать **sticky sessions** для долгоживущих соединений (например, уроков), чтобы запросы одного пользователя всегда отправлялись на один и тот же сервер.

#### Оптимизация и отказоустойчивость:
- Для оптимизации времени отклика внедрим механизм возобновления **SSL-сессий** с использованием session tickets, что уменьшит количество полных рукопожатий TLS.

- Использование **Active-Active кластеров** позволит обрабатывать запросы всеми узлами одновременно. В случае выхода из строя одного из серверов, другие продолжат обработку запросов, не влияя на пользователя.

- Для мониторинга состояния используем системы вроде **Grafana**.
---
